...
export const CrossHeaderTab = React.forwardRef((props: CrossHeaderTabType, ref) => {
  ...
  tabContentOffsets.forEach((contentOffset, index) => {
    useAnimatedReaction(
      ...
      (res, pre) => {
        if (pre >= headerSize.height.value - screenHeight - 50 || res === pre) return;
  ...
  return (
    <View style={{ flex: 1, overflow: "hidden" }}>
      ...
      >
        {arr.map((_, index) => {
          return (
            <CrossHeaderTabContext.Provider key={index} value={childrenPanActions[index]}>
              {props.renderTab(index)}
            </CrossHeaderTabContext.Provider>
          );
        })}
      </SpringScrollView>
      <SpringScrollView size={headerSize} style={headerStyle} panHandler={panHandler}>
        {props.renderHeader()}
